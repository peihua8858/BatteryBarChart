...
class BarChartTouchHelper(private val hostView: BatteryBarChart) : ExploreByTouchHelper(hostView) {
    public override fun getVirtualViewAt(f10: Float, f11: Float): Int {
        for (batteryStackBarData in hostView.mBarLists) {
            ...
            if (f12 <= f10 && f12 + batteryStackBarData.width >= f10) {
                return batteryStackBarData.state
            }
        }
        return -1
    }
        ...
        if (i4 >= 0) {
            ...
            if (i4 <= batteryBarChart.mBarLists.size) {
                ...
                if (batteryBarChart.startIndex < batteryBarChart.mBarLists.size && batteryBarChart.endIndex < batteryBarChart.mBarLists.size) {
                    val f10 = batteryBarChart.mBarLists[batteryBarChart.startIndex].x
                    val f11 =
                        batteryBarChart.mBarLists[batteryBarChart.endIndex].x + batteryStackBarData.width
